...
class Viewer_Parser_If
{
    ...
    public function parse($token)
    {
        ...
        $this->parser->getStream()->expect(Viewer_Token::BLOCK_END_TYPE); // если не текущий токен не соответствует типу, выполнение прерывается
        ...
        while (false === $end) {
            switch ($this->parser->getStream()->next()->getValue()) {
                case 'else':
                    $this->parser->getStream()->expect(Viewer_Token::BLOCK_END_TYPE);
        ...
        return new Viewer_Node_If($tests, $else);
    }
    ...
    public function endWork(Viewer_Token $token)
    {
