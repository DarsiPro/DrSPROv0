...
Class ActionsHandler extends \Module {
    ...
    public function add() {
        ...
        if (empty($title))
            $error .= '<li>' . sprintf(__('Empty field "param"'), __('Stat title')) . '</li>' . "\n";
        elseif (!\Validate::cha_val($title, V_TITLE))
            $error .= '<li>' . sprintf(__('Wrong chars in field "param"'), __('Stat title')) . '</li>' . "\n";
    ...
    public function update($id = null) {
        ...
        if (!isset($id)
            ...
            return $this->showMessage(__('Some error occurred'),getReferer());
        }
        if (!empty($id) && !is_numeric($id))
            return $this->showMessage(__('Value must be numeric'));
        if ($id < 1)
